...
## Локальный Docker-запуск с доступом к SQL Server

Если нужно подключаться к базе данных с хоста, используйте локальный override:

```bash
docker compose -f docker-compose.yml -f docker-compose.local.yml up --build -d
```

В этом режиме SQL Server будет доступен на хосте:
...
```text
Server=localhost,14330;Database=PasswordManagerDb;User Id=sa;Password=YOUR_SA_PASSWORD;TrustServerCertificate=True;
```

## Миграции базы данных

В текущей Docker-конфигурации миграции Entity Framework не применяются автоматически. После первого запуска базы данных или после добавления новых миграций выполните:

```bash
dotnet ef database update --project PasswordManagerAPI --startup-project PasswordManagerAPI --connection "Server=localhost,14330;Database=PasswordManagerDb;User Id=sa;Password=YOUR_SA_PASSWORD;TrustServerCertificate=True;"
```

Перед выполнением команды SQL Server должен быть запущен через `docker-compose.local.yml`, чтобы порт `14330` был доступен на хосте.

Если команда `dotnet ef` недоступна, установите инструмент:
