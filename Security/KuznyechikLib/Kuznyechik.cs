...
namespace KuznyechikLib
{
    public class Kuznyechik
    {
        ...
        static Kuznyechik()
        {
            BuildRoundTables(false, EncryptRoundTableLow, EncryptRoundTableHigh);
        }
        ...
        private void DecryptBlockFast(ReadOnlySpan<byte> input, Span<byte> output)
        {
            Span<byte> state = stackalloc byte[Transformations.BLOCK_SIZE];
            Span<byte> linear = stackalloc byte[Transformations.BLOCK_SIZE];

            ulong low = BinaryPrimitives.ReadUInt64LittleEndian(input[..8]) ^ roundKeyLow[9];
            ulong high = BinaryPrimitives.ReadUInt64LittleEndian(input[8..]) ^ roundKeyHigh[9];
            BinaryPrimitives.WriteUInt64LittleEndian(state[..8], low);
            BinaryPrimitives.WriteUInt64LittleEndian(state[8..], high);

            for (int round = 8; round >= 0; round--)
            {
                Transformations.ApplyLInverseMatrixTables(state, linear);

                for (int i = 0; i < Transformations.BLOCK_SIZE; i++)
                {
                    state[i] = (byte)(Transformations.ApplyInverseSBox(linear[i]) ^ iteration_keys[round][i]);
                }
            }
        ...
        private static void BuildRoundTables(bool inverse, ulong[] tableLow, ulong[] tableHigh)
        {
            for (int position = 0; position < Transformations.BLOCK_SIZE; position++)
            {
                for (int value = 0; value < 256; value++)
                {
                    ...
                    vector[position] = inverse
                        ? Transformations.ApplyInverseSBox((byte)value)
                        : Transformations.ApplySBox((byte)value);

                    byte[] transformed = inverse
                        ? Transformations.LInverse(vector, LinearTransformImplementation.MatrixTables)
                        : Transformations.L(vector, LinearTransformImplementation.MatrixTables);
