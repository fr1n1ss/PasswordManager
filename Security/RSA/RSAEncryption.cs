    ...
    public class RSAEncryption
    {
        ...
        public string DecryptText(string encryptedText)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(result) || result.Any(c => char.IsControl(c) && c != '\n' && c != '\r' && c != '\t'))
                {
                    throw new ArgumentException("Расшифрованный текст повреждён. Возможно, использованы неверные ключи.");
                }
                return result;
            }
            catch (DecoderFallbackException)
            {
                throw new ArgumentException("Не удалось декодировать текст. Возможно, использованы неверные ключи.");
            }
