...
namespace KuznyechikLib
{
    ...
    public class Transformations
    {
        ...
        private static readonly byte[] L_VECTOR = [148, 32, 133, 16, 194, 192, 1, 251, 1, 192, 194, 16, 133, 32, 148, 1];
        ...
        public static byte[] S(byte[] inputData)
        {
            byte[] result = new byte[inputData.Length];
            for (int i = 0; i < BLOCK_SIZE; i++)
            {
                result[i] = PI[inputData[i]];
            }
            return result;
        }

        public static byte ApplySBox(byte value) => PI[value];

        public static byte[] SInverse(byte[] inputData)
        {
            byte[] result = new byte[inputData.Length];
            for (int i = 0; i < BLOCK_SIZE; i++)
            {
                result[i] = PI_INVERSE[inputData[i]];
            }
            return result;
        }

        public static byte ApplyInverseSBox(byte value) => PI_INVERSE[value];

        public static byte[] L(byte[] inputData, LinearTransformImplementation implementation = LinearTransformImplementation.Standard)
        {
            return ApplyLStandard(inputData);
        }

        public static byte[] LInverse(byte[] inputData, LinearTransformImplementation implementation = LinearTransformImplementation.Standard)
        {
            return ApplyLInverseStandard(inputData);
        }

        public static byte[] R(byte[] inputData)
        {
            ...
            for (int i = 0; i < BLOCK_SIZE; i++)
            {
                ...
                b0 ^= MULTIPLY_TABLE[inputData[i], L_VECTOR[i]];
            }
        ...
        public static byte[] RInverse(byte[] inputData)
        {
            byte[] result = new byte[BLOCK_SIZE];
            byte sum = 0;
